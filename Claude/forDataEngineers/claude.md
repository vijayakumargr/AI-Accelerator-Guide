...
## Database-Specific Parallelism
- **BigQuery**: Use partitioned tables and clustered columns for automatic parallelism
- **Snowflake**: Leverage warehouse auto-scaling and clustering
- **Redshift**: Use DISTSTYLE and SORTKEY for parallel query execution
- **Spark**: Configure appropriate partition counts based on data volume
...
- Implement the exact approach we discussed
- If you discover data quality issues, stop and discuss
- If performance is significantly different than expected, flag it

## Documentation Requirements
- Schema definitions with column descriptions
- Data lineage (where does data come from, where does it go)
- Business logic explanations for complex transformations
...
- Don't jump to writing SQL without understanding the data
- Don't make schema decisions unilaterally
- Don't start responses with praise
- Don't validate every decision as "absolutely right"
- Don't agree just to be agreeable
- Don't treat subjective preferences as objective improvements

## Data-Specific Anti-Patterns
- **NEVER deploy without testing on sample data first**
- **NEVER modify production data without a backup/rollback plan**
- **NEVER assume data types** - verify from source
- **NEVER ignore failed records** - handle or log them explicitly
- **NEVER use dynamic SQL with user input** without parameterization
- **NEVER truncate and reload** when incremental is feasible and safer
...
## Assume Competency
- Assume I understand SQL, data modeling, and pipeline concepts
- Don't over-explain basic concepts unless asked
- Focus on trade-offs and edge cases

## Be Direct About Issues
- Point out inefficient query patterns immediately
- Flag data quality risks without excessive hedging
- Challenge suboptimal pipeline designs directly
- Identify missing error handling or monitoring

---

# Context About Me

- Data engineer with experience across multiple data platforms
- Prefer thorough planning to minimize pipeline failures
- Want to be consulted on modeling and architecture decisions
- Comfortable with technical discussions and constructive feedback
- Looking for genuine technical dialogue, not validation
- Value reliability and maintainability over clever solutions
...
## BigQuery
- Use `PARTITION BY` for date-based filtering efficiency
- Use `CLUSTER BY` for frequently filtered columns
- Prefer `MERGE` statements for upserts
- Be aware of slot consumption for cost management
- Use `INFORMATION_SCHEMA` for metadata queries

## Snowflake
- Leverage Time Travel for recovery scenarios
- Use appropriate warehouse sizes for workload
- Implement Zero-Copy Cloning for development
- Use Streams and Tasks for CDC patterns
- Be mindful of credit consumption

## Databricks/Spark
- Choose appropriate file formats (Delta, Parquet)
- Configure partitioning based on query patterns
- Use broadcast joins for small dimension tables
- Monitor Spark UI for performance bottlenecks
- Implement proper caching strategies

## Redshift
- Use appropriate DISTSTYLE (KEY, ALL, EVEN)
- Define SORTKEY based on query patterns
- Vacuum and analyze tables regularly
- Use Spectrum for external data
- Monitor WLM queues for concurrency
